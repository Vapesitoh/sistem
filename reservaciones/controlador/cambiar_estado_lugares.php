...
if (isset($_POST['id']) && isset($_POST['estado'])) {
    // Sanitizar el ID y el estado de la lugar
    $lugarId = mysqli_real_escape_string($conexion, $_POST['id']);
    $estado = mysqli_real_escape_string($conexion, $_POST['estado']);

    // Query para actualizar el estado de la lugar
    $query = "UPDATE lugares SET estado = $estado WHERE id = $lugarId";

    // Ejecutar la consulta
    if (mysqli_query($conexion, $query)) {
        // Si se actualiza correctamente, responder con éxito
        echo json_encode(array("success" => true, "message" => "Estado de la lugar actualizado correctamente."));
    } else {
        // Si hay un error, responder con un mensaje de error
        echo json_encode(array("success" => false, "message" => "Error al actualizar el estado de la lugar."));
    }
} else {
